...
function renderSystemPlan(){
 ...
 const v=ensureSystemPlan(),tab=v.planTab==='ai'?'ai':'my',selected=v.selectedDate||ymd(today()),hasAI=tab!=='ai'||!!v.aiMeta?.[selected],raw=planFor(selected,tab),p=hasAI?raw:{type:'AI',name:'No AI recommendation yet',duration:0,location:'KINETIQ Coach',intensity:'—',ids:[],textExercises:[]},base=today(),dates=Array.from({length:7},(_,i)=>addDays(base,i)),d=new Date(selected+'T12:00:00'),meta=aiPlanMeta(selected,p);
 ...
 root.innerHTML=`<div class="system-page" data-system-screen="plan"><div class="system-tabs"><button class="${tab==='my'?'active':''}" data-system-tab="my">MY PLAN</button><button class="${tab==='ai'?'active':''}" data-system-tab="ai">AI RECOMMENDED</button></div><div class="system-days">${dates.map(x=>{const k=ymd(x);return`<button class="system-day ${k===selected?'active':''}" data-system-date="${k}"><small>${new Intl.DateTimeFormat('en-GB',{weekday:'short'}).format(x).toUpperCase()}</small><b>${x.getDate()}</b></button>`}).join('')}</div><section class="system-plan-card ${tab==='ai'?'ai':''}"><div class="system-plan-top"><div><div class="system-kicker">${tab==='my'?'MY PLAN':'AI RECOMMENDED'}</div><div class="system-plan-date">${esc(compactDate(d).toUpperCase())}</div></div><span class="system-plan-location">${esc(p.location||p.type||'Flexible')}</span></div><h1>${esc(p.name||'No session')}</h1><div class="system-plan-meta"><span><b>${esc(p.duration||v.duration||45)}</b><small>MIN</small></span><span><b>${esc((p.intensity||'Moderate').toUpperCase())}</b><small>INTENSITY</small></span><span><b>${esc((p.type||'Training').toUpperCase())}</b><small>TYPE</small></span></div>${tab==='ai'&&hasAI?`<div class="system-plan-why"><small>WHY THIS SESSION</small><p>${esc(meta.reason)}</p></div>`:tab==='ai'?`<div class="system-plan-why"><small>AI RECOMMENDED</small><p>Ask KINETIQ Coach to create a recommendation for this date. My Plan remains unchanged until you apply it.</p></div>`:''}<div class="system-ex-list">${exerciseRows}${textRows}</div>${p.run?`<div class="system-run-plan"><b>${esc(p.run.kind||'Run')}</b><span>${esc(p.run.distance||'')}</span></div>`:''}<div class="system-plan-actions">${tab==='ai'?(hasAI?'<button class="system-primary" data-system-apply-selected>APPLY TO PLAN</button><button class="system-secondary" data-system-keep-selected>KEEP CURRENT PLAN</button>':'<button class="system-primary" data-system-ai>ASK KINETIQ COACH →</button>'):actionForPlan(p,selected)}</div></section><section class="system-card system-coach-cta"><div><div class="system-kicker">KINETIQ COACH</div><h3>Need to change this day?</h3><p class="system-sub">Tell the coach what changed. My Plan stays untouched until you approve a recommendation.</p></div><button class="system-secondary" data-system-ai>OPEN AI COACH</button></section></div>`;
