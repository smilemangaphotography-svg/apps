...
new_guard = '''        if (!permanentSource.contains('v.setPadding(bars.left, bars.top, bars.right, bars.bottom)')) throw new GradleException('Permanent Merchant native inset ownership missing')
        ...
        if (!permanentSource.contains('slm-native-layout-136')) throw new GradleException('Permanent Merchant three-row layout missing')
        if (!permanentSource.contains('position:relative!important')) throw new GradleException('Permanent Merchant bottom navigation flow lock missing')
        if (!permanentSource.contains('overflow-y:auto!important')) throw new GradleException('Permanent Merchant scrolling fix missing')'''
text = replace_once(text, old_guard, new_guard, 'merchant Gradle runtime guard')
p.write_text(text, encoding='utf-8')
