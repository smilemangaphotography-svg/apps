...
/* NAR BETA 5.0.7 — ALL BRANDS CAPTURE GUARD
   A legacy Home handler can still react after the visible Edit control is
   renamed to All Brands. Intercept that control before legacy bubbling and
   open the canonical 5.0.7 pin selector directly. */
(function(){
  ...
  function showSelector(){
    ...
    const html=`<div class="sheethead"><button id="nar507PinsBack" class="backbtn">← Home</button><span>All Brands</span></div>
    ...
    const back=document.querySelector('#nar507PinsBack');if(back)back.onclick=()=>{if(typeof closeModal==='function')closeModal()};
...
print('Applied NAR Beta 5.0.7 All Brands capture guard')
