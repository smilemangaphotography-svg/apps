(function(){
  ...
  saveCustomMaster=function(){const name=$id('customName').value.trim(),prompt=$id('customPrompt').value.trim(),ratio=$id('customRatio').value,preserve=$id('customPreserve').value,preset=$id('customPreset').checked,quick=$id('customQuick').checked;if(!name||!prompt){toast('Name and command are required.');return}const m={id:'custom-'+Date.now(),name,prompt,ratio,preserve,preset,quick};state.customMasters.push(m);localStorage.setItem('pmai_masters',JSON.stringify(state.customMasters));$id('customName').value='';$id('customPrompt').value='';$id('customQuick').checked=false;renderMasters(false);toast('Master saved.')};
  renderMasters=function(presetsOnly=false){const box=$id('masterList');if(!box)return;const list=state.customMasters.filter(m=>!presetsOnly||m.preset);if(!list.length){box.innerHTML='<div class="small-note">'+(presetsOnly?'No saved presets yet.':'Your saved master commands will appear here.')+'</div>';return}box.innerHTML='';[...list].sort((a,b)=>(b.quick?1:0)-(a.quick?1:0)).forEach(m=>{const d=document.createElement('div');d.className='master-item';d.innerHTML=`<div><b>${escapeHtml(m.name)}</b><small>${escapeHtml(m.ratio)} · ${escapeHtml(m.preserve||'Strict')} ${m.preset?'· Preset':''} ${m.quick?'· Quick Access':''}</small></div><div><button onclick="runCustom('${m.id}')">Use</button><button onclick="deleteCustom('${m.id}')">×</button></div>`;box.appendChild(d)})};
  window.showSavedPresets=function(){showScreen('custom');renderMasters(true)};
